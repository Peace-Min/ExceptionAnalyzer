...
| `2` | **부분 결과** — 일부 파일/프로젝트가 무결성 문제(워크스페이스 로드 실패·baseline 컴파일 오류·문서 스킵)로 스킵됨. **⚠️ `--apply`에서 깨끗한 파일은 이미 적용되었고, 오염 파일만 스킵된 상태** — "아무것도 안 바뀜"이 아니므로 반드시 작업트리 diff를 검토할 것. fix-report의 `SkippedIntegrityFiles`·`AppliedFiles` 확인. (커버리지 부족은 비차단, 여기 해당 안 됨. `--source-only`만 완전 진단 전용) |
...
apply는 **파일 단위 best-effort** 입니다 — 문제 있는 파일만 스킵하고, 깨끗한 파일은 적용합니다:

| 분류 | 예 | apply 영향 |
|---|---|---|
| **무결성 문제(해당 파일만 스킵)** `SkippedIntegrityFiles` | 그 파일의 baseline 컴파일 오류, 소속 프로젝트 로드 실패, 문서 스킵 | **그 파일만 적용 제외** — 나머지 깨끗한 파일은 적용됨. 하나라도 스킵되면 전체 결과는 exit 2(PARTIAL)로 보고되지만 **깨끗한 파일 apply는 이미 수행됨** |
| **`--source-only` 정책** | `.sln` 없이 폴더 파일 스캔 | **완전 진단 전용** — 프로덕션에서 아무 파일도 적용 안 함 |
...
> ⚠️ **exit 2 ≠ "아무것도 안 바뀜".** 부분 적용(PARTIAL)에서도 깨끗한 파일은 이미 수정됩니다. `--apply` 후엔 항상 작업트리 diff를 검토하세요(git 권장).
...
- **파일 단위 best-effort** — Completeness=**PARTIAL**(exit 2)은 일부 파일/프로젝트가 무결성 문제로 스킵됐다는 뜻이며, **깨끗한 파일 apply는 그대로 수행됨**(전역 차단 아님). `SkippedIntegrityFiles`에 스킵 사유, `AppliedFiles`에 실제 적용 수. **커버리지 참고(비차단)는 apply를 막지 않음.** `--source-only`만 완전 진단 전용
