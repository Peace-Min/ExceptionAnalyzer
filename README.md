...
| `2` | **부분 결과** — 일부 프로젝트/문서 로드 실패 또는 coverage warning 발생. `--apply`는 수행하지 않으며 fix-report 확인 |
...
6. **재작성** — 구체 catch를 원본 broad catch 앞에 삽입하고 broad fallback은 보존(원본 try 본문·finally·트리비아 보존), 변경 노드만 포맷(파일 전체 재포맷 없음), **원본 인코딩(BOM/UTF-16/CP949)·개행(CRLF/LF) 보존** 및 임시 파일 교체 저장.
...
- Completeness=PARTIAL(exit 2)이면 결과를 **완전한 목록으로 간주하지 말 것**. 적용 모드에서는 자동수정이 차단됨. PARTIAL 사유는 WORKSPACE FAILURES와 COVERAGE WARNINGS에 기록됨
...
| `Program.SelfTest.cs` | 자가 검증 | `RunSelfTestHeadless`, `RunSelfTestFix`([1]~[21] 픽스처 하니스) |
...
dotnet run --project ExceptionAnalyzer/ExceptionAnalyzer.csproj -- --selftest-fix   # 재작성 [1]~[21]
